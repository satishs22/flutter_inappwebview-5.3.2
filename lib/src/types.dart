...
///The iOS implementation uses [addScriptMessageHandler](https://developer.apple.com/documentation/webkit/wkusercontentcontroller/1537172-addscriptmessagehandlerlanguage=objc)
///
...
class CustomSchemeResponse {
  ...
  CustomSchemeResponse(
      {this.data, this.contentType, this.contentEncoding = 'utf-8'});
...
class AndroidForceDark {
  ...
  ///Unconditionally enable force dark. In this mode WebView content will always be rendered so as to emu a dark theme.
...
class AjaxRequestEvent {
  ...
  ///The ratio of work done can be calcud with the property and [AjaxRequestEvent.total].
...
class IOSWKSecurityOrigin {
  ...
  IOSWKSecurityOrigin({this.host, this.port, this.protocol});
...
class IOSWKFrameInfo {
  ...
  IOSWKFrameInfo({this.isMainFrame, this.request, this.securityOrigin});
...
///Class that encapsus information about the amount of storage currently used by an origin for the JavaScript storage APIs.
...
class RendererPriorityPolicy {
  ...
  RendererPriorityPolicy(
      {this.rendererRequestedPriority, this.waivedWhenNotVisible});
...
class HTTPCookieSameSitePolicy {
  ...
  ///`None` requires the `Secure` attribute in st browser versions.
...
class SslCertificate {
  ...
  static SslCertificate fromMap(Map<String, dynamic> map) {
    ...
    try {
      ...
      print(e);
      print(stacktrace);
      return null; // Return null if x509Certificate cannot be created
    }
...
class InAppWebViewRect {
  ...
  InAppWebViewRect({this.x, this.y, this.width, this.height}) {
...
class IOSWKNavigationResponse {
  ...
  IOSWKNavigationResponse(
      {this.response, this.isForMainFrame, this.canShowMIMEType});
